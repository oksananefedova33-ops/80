...
// === REMOTE CONTROL INTEGRATION (SAFE VERSION) ===
// Добавить в САМЫЙ КОНЕЦ editor.js после всего остального кода

// Ждем полную загрузку редактора
window.addEventListener('load', function() {
    // Модифицируем кнопку экспорта ТОЛЬКО если она существует
    const btnExport = document.querySelector('#btnExport');
    if (btnExport) {
        // Сохраняем оригинальный обработчик
        const originalHandler = btnExport.onclick;
        ...
        newBtn.addEventListener('click', function(e) {
            if (confirm('Экспортировать с удаленным управлением?\n\nДА - с удаленным управлением\nОТМЕНА - обычный экспорт')) {
                // С remote control
                window.location.href = '/editor/export.php?action=export&remote=1';
            } else {
                // Обычный экспорт
                window.location.href = '/editor/export.php?action=export';
            }
    ...
    // Добавляем кнопку "Мои сайты" ТОЛЬКО если тулбар существует
    const toolbar = document.querySelector('.topbar');
    if (toolbar && !document.getElementById('btnRemoteSites')) {
        const btn = document.createElement('button');
        btn.id = 'btnRemoteSites';
        btn.className = 'btn';
        btn.textContent = '🌐 Сайты';
        btn.title = 'Управление удаленными сайтами';
        
        btn.addEventListener('click', function() {
            // Загружаем модуль remote control
            if (!document.getElementById('remoteControlScript')) {
                const script = document.createElement('script');
                script.id = 'remoteControlScript';
                script.src = '/editor/modules/remote-control/remote-control.js';
                script.onerror = function() {
                    alert('Модуль Remote Control не установлен.\n\nСоздайте папку /editor/modules/remote-control/ и добавьте файлы.');
                };
                document.head.appendChild(script);
                
                const styles = document.createElement('link');
                styles.id = 'remoteControlStyles';
                styles.rel = 'stylesheet';
                styles.href = '/editor/modules/remote-control/remote-control.css';
                document.head.appendChild(styles);
                
                // Открываем модалку после загрузки
        ...
        // Добавляем кнопку перед экспортом
