...
header('Access-Control-Allow-Headers: X-API-Token, X-Action, X-File-Url, X-Old-Url, X-New-Url, X-File-Name');
header('Content-Type: application/json; charset=utf-8');
...
function searchFileInPages($fileUrl) {
    $found = [];
    $htmlFiles = glob('../*.html');
    
    foreach ($htmlFiles as $file) {
        $content = file_get_contents($file);
        if (strpos($content, $fileUrl) !== false) {
            $found[] = basename($file);
        }
    }
...
function replaceInPages($oldUrl, $newUrl, $fileName = '') {
    $count = 0;
    $htmlFiles = glob('../*.html');
    ...
    foreach ($htmlFiles as $file) {
        ...
        // Если указано имя файла, обновляем и атрибут download
        if ($fileName && strpos($content, 'download="') !== false) {
            $newContent = preg_replace(
                '/download="[^"]*"([^>]*href="' . preg_quote($oldUrl, '/') . '")/i',
                'download="' . $fileName . '"$1',
                $newContent
            );
        }
...
switch($action) {
    ...
    case 'search-file':
        ...
        $found = searchFileInPages($fileUrl);
    ...
    case 'replace-link':
        ...
        $result = replaceInPages($oldUrl, $newUrl);
        echo json_encode(['success' => true, 'replaced' => $result]);
        break;
